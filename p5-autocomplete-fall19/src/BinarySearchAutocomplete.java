...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		if (last == -1) {               // prefix not found
			return new ArrayList<>();
		}

		Term[] kTerms = new Term[last-first+1];

		for (int i = 0; i < last-first+1; i++) { //might have to say +1
			kTerms[i] = new Term(myTerms[count].getWord(), myTerms[count].getWeight());
			count++;
		}
